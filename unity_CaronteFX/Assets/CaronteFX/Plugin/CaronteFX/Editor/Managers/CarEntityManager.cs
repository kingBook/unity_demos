...
namespace CaronteFX
{
  ...
  using ParamaterModifierAction = CaronteFX.Tuple4< UN_BD_GENERIC_FAMILY, uint, ICarParameterEvaluator, ParameterModifierCommand >;
  ...
  public class CarEntityManager
  {
    ...
    private uint CreateBody(CNRope rpNode, GameObject go, out Mesh meshDefinition, out Mesh meshTile, out bool isBakedDefinitionMesh)
    {
      ...
      if (hasBodyComponent)
      {
        ...
        CarBodyUtils.GetDefinitionAndTileMeshes(go, ref meshDefinition, out m_Definition_Local_To_World, ref isBakedDefinitionMesh, ref meshTile);
    ...
    private PmInit GetParameterModifierInit( CNParameterModifier pmNode, uint[] arrIdBody, uint[] arrIdMultiJoint, uint[] arrIdServos, uint[] arrIdEntity )
    {
      ...
      for (int i = 0; i < listPmCommand_size; i++)
      {
        ...
        CarDataUtils.SetParameterModifierCommand(ref arrPmCommand[i], parameterModifierCommand);
      }
    ...
    private CorpusclesInit GetCorpusclesInit(CNCorpuscles cpNode, GameObject[] arrDefinitionGameObject)
    {
      ...
      CarGeometryUtils.CreateTransformedAppendedMeshSimple(arrDefinitionGameObject, out definitionMesh);
    ...
    //-----------------------------------------------------------------------------------
    public List<ParamaterModifierAction> GetListParameterModifierAction(CommandNode[] arrCommandNode)
    {
      List<ParamaterModifierAction> listParameterModifierAction = new List<ParamaterModifierAction>();
      ...
      foreach(CommandNode commandNode in arrCommandNode)
      {
        ...
        if ( svNode != null && tableServosToId_.ContainsKey(svNode) )
        {
          if ( !svNode.TargetExternal_LOCAL_NEW.IsConstant() )
          {
            ...
            if (idServos != INDEX_NOT_FOUND)
            {
              ParamaterModifierAction pmAction = new ParamaterModifierAction( UN_BD_GENERIC_FAMILY.UN_BD_GENERIC_FAMILY_SERVO,
                                                                              idServos, 
    ...
    {
      int instanceId = go.GetInstanceID();
      CarCreationData crData = new CarCreationData(go, renderMesh, colliderMesh, btAsset, type == BodyType.Ropebody);
